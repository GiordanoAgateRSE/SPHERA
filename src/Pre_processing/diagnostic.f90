...
!------------------------
write(error,'(i2)') ierr
write(uerr,'(a,a2,a)') ' >> Diagnostic ',error,                                &
   ' detected * Run ended unsuccessfully'
if (ierr/=1) then
   write(ulog,'(1x,100(''=''))')
   write(ulog,*)
   write(ulog,'(a,a2,a)') 'Diagnostic ',error,                                 &
      ' detected * Run ended unsuccessfully'
   write(ulog,*)
   flush(ulog)
endif
...
" Correct command sequence-> <executablename> <casename> [euristic]|[original]"
   case (2)
! Error in files
      write(ulog,'(1x,a)') 'Checkfile function detected an error:'
      select case (dato)
         case (1); write(ulog,'(1x,a)') '  1 - Output file cannot be opened.'
         case (2); write(ulog,'(1x,a)') '  2 - Input file does not exist.'
         case (3); write(ulog,'(1x,a)') '  3 - Input file cannot be opened.'
      endselect
   case (3)
! Error in arrays
      write(ulog,'(1x,2a)') 'Array deallocation failed.  Routine -> ',         &
         stringa(1:la)
      write(ulog,'(1x,a)') 'Run is stopped.'
   case (4)
! Error in arrays
      write(ulog,'(1x,2a)')                                                    &
...
! Error during the simulation
      write(ulog,'(1x,a)') 'An error was detected during the loop'
      write(ulog,'(1x,a)') 'Specific code error is:'
      select case (dato)
         case (1); write(ulog,'(1x,a,i10,2a)')                                 &
...
! Error during the execution of auxiliary subroutines
      write(ulog,'(1x,a)') 'An error was detected during the tools execution'
      write(ulog,'(1x,a)') 'Specific code error is:'
      select case (dato)
         case (1); write(ulog,'(1x,a,i10,2a)')                                 &
...
if (print_out) then
   write(ulog,'(1x,(a))') ' ---------------------------'
   write(ulog,'(1x,(a))') ' Print results at last step.'
   write(ulog,'(1x,(a))') ' ---------------------------'
   write(uerr,'(1x,(a))') ' ---------------------------'
   write(uerr,'(1x,(a))') ' Print results at last step.'
   write(uerr,'(1x,(a))') ' ---------------------------'
   it1 = 0
   it2 = 0
   it3 = 0
