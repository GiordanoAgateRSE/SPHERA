...
! Time independent information on solid bodies
   if (n_bodies>0) then
      write(nres) n_bodies_CAE,n_body_part_CAE
   endif
#endif
   flush(nres)
   write(ulog,'(a)')                                                           &
...
! Only for the first and last steps
   ...
   if (allocated(surf_body_part)) write(nres) surf_body_part(1:n_surf_body_part)
      ...
      if (allocated(z_topog_max)) write(nres) z_topog_max(1:size(z_topog_max))
   ...
   if (index(str,'inizio')==0) then
      ...
      write(ulog,'(a)')                                                        &
      " --------------------------------------------------------------------"
      call system("cp -p "//nomefile(2)//" restart_tjob_0.ris0")
   endif
