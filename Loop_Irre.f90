...
!AA504
! 06  Amicarelli        08Apr14        (v5.04) Minor modifications related to: elapsed time estimation, erosion criterion, density thresholds, mixture viscosity,  
!                                      output for granular flows, hydrographs, maximum depth. 
! 07  Amicarelli        26Jan15        DBSPH-input (AA601). Several modifications (e.g.: calling DBSPH post-processing, removing fictitious air reservoirs,...)
!
!************************************************************************************
! Module purpose : Module Loop
!
...
!.. Local Scalars ..
  integer(4)         :: Ncbf
  integer(4)         :: i, npi, it, it_print, it_memo, it_rest, ir, ii
  integer(4)         :: num_out, Ncbf_Max
!AA504 sub
  integer(4)         :: OpCountot,SpCountot,EpCountot,EpOrdGridtot,ncel,aux,igridi,jgridi,kgridi,machine_Julian_day,machine_hour,machine_minute,machine_second
!AA504 sub
  double precision   :: pretot,BCtorodivV,DtPreviousStep,TetaV1,xmax,ymax,zmax,appo1,appo2,appo3,dtvel
!
...
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
!
!.. inizializzazione vettore appoggio per conteggio particelle non 'sol' caso con 'granular'
!
!  indarraySol = 0
  indarrayFlu = 0
  do npi = 1,nag
...
!.. evaluates the distances between the one particle and all the nearest ones, storing them 
!.. in the general storage array
!.. computation and storage of neighbouring indices, distances and kernel gradients 
!
  call start_and_stop(2,10)
!AA601
  if ((it_corrente==it_start).and.(Domain%tipo=="bsph")) it_corrente = -2
...
!.. evaluates the close boundaries and the integrals
!.. for the current particle in every loop, storing them in the general storage array
!.. computaion of the boundary integrals
!
!AA406 sub start
  if (Domain%tipo == "semi") then
     call start_and_stop(2,11)
...
!§ 
!
!.. Salvataggio risultati e restart INIZIALE (sempre?)
!
  if ( nout > 0) then
    it_print = it_eff
    call print_results ( it_eff, it_print, 'inizio' )
...
!
    DtPreviousStep = dt
!.. CFL condition
    if (nag > 0) call rundt2      !$$$$$$$$$$ VERIFICARE IL SENSO DEL TEST $$$$$$$$$$$$$
!
    tempo = tempo + dt
!
    if ( nscr > 0 ) write (nscr,"(a,i8,a,g13.6,a,g12.4,a,i8,a,i5)") " it= ",it,"   time= ",tempo,"  dt= ",dt, &
                                "    npart= ",nag,"   out= ",num_out    !AdM_U7+
!
!    if ( nout > 0 ) then
!       if ( it == 1 ) then
!          write (nout,"(a,i8,a,g12.4,a,g12.4,a,i5,a,i8,a,i8)") &
!                " it=",it, "     tempo=",tempo,"    dt=",dt,"   out=",num_out,"    npart=",nag
!       else if ( Domain%ioutopt == 0 ) then  ! nulla
!       else if ( Domain%ioutopt <  0 ) then  ! completa
!          write (nout,"(a,i8,a,g12.4,a,g12.4,a,i5,a,i8,a,i8)") &
!                " it=",it, "     tempo=",tempo,"    dt=",dt,"   out=",num_out,"    npart=",nag
!       else if ( mod(it,iabs(Domain%ioutopt)) == 0 ) then  !parziale
!          write (nout,"(a,i8,a,g12.4,a,g12.4,a,i5,a,i8,a,i8)") &
!                " it=",it, "     tempo=",tempo,"    dt=",dt,"   out=",num_out,"    npart=",nag
!       end if
!    end if
...
!
        call vellaw(Partz(ir)%vlaw,Partz(ir)%vel,Partz(ir)%npointv)
!
        write(nout,"(f12.4,a,i2,a,3e12.4)") tempo,"  zona",ir,"  vel.",Partz(ir)%vel
        do npi = Partz(ir)%limit(1),Partz(ir)%limit(2) !!! TRASFORMARE il DO sulle particelle per eliminare le zone
          if ( pg(npi)%cella == 0 ) cycle
          pg(npi)%var(:) = Partz(ir)%vel(:)
          if ( tempo >= pg(npi)%tstop ) then
...
!
    call start_and_stop(2,6)
!!!tempi    call cpu_time(cpu_prima)
!
!$omp parallel do default(none) &
!$omp private(npi,ii,tpres,tdiss,tvisc,ncbf,boundreaction) &
!$omp shared(nag,pg,Domain,BoundaryDataPointer,Ncbf_Max,indarrayFlu,Array_Flu,it)
!
!.. loops on all the particles
!
!!!!!!    do npi = 1,nag
!!!!!!      if ( pg(npi)%cella == 0 .or. pg(npi)%vel_type /= "std" .or. pg(npi)%state == "sol") cycle
!$$$$$
!.. loop for the right hand side of the momentum equation
    do ii = 1,indarrayFlu
      npi = Array_Flu(ii)
!$$$$$
!
...
!
      call inter_EqMoto (npi,tpres,tdiss,tvisc)  
!
!.. searches for the boundary faces closest to the i-th current particle
!
!AA404 sub
      if ((Domain%time_stage == 1) .or. (Domain%time_split == 1)) then 
!
...
!
!.. closest faces are really present
!AA406 sub
...
!$omp end parallel do
!
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (89,'(a,i5,a,f14.7)') " step ",it_corrente," * Eq. moto tot ",cpu_dopo - cpu_prima
!
    if ( Ncbf_Max > Domain%MAXCLOSEBOUNDFACES ) then
      write (nout,"(a,i5,a,i5)") "Increase parameter MAXCLOSEBOUNDFACES from",Domain%MAXCLOSEBOUNDFACES," to ",Ncbf_Max
...
!
!
!.. upgrading of the velocity. Evaluation of the average dt 
!
!    dtvel = half * (dt + dtm) 
!      
!.. evaluates the final motion field in terms of final velocity components 
...
!!!tempi    call cpu_time(cpu_prima)
!
!$omp parallel do default(none) private(npi,ii) shared(nag,Pg,dtvel,indarrayFlu,Array_Flu)
!
!.. loops on all the particles
!
!!!!!!    do npi = 1,nag
...
!!!!!!      else
!$$$$$
!.. time integration of the momentum equation
      do ii = 1,indarrayFlu
        npi = Array_Flu(ii)
!$$$$$
!
...
!..             serie development, being DV/Dt = acceleration components
        if (pg(npi)%kodvel == 0) then 
          pg(npi)%vel(:) = pg(npi)%vel(:) + dtvel * pg(npi)%acc(:)
!
!.. ????? controllare     kodvel = 1: the particle has a critical flux condition. The horizontal velocity components are set using the 
!..             serie development, while the vertical one is assigned
        else if (pg(npi)%kodvel == 1) then                                                    
          pg(npi)%vel(:) = pg(npi)%vel(:) + dtvel * pg(npi)%acc(:)                            
          pg(npi)%vel(3) = pg(npi)%velass(3)
...
!
      call start_and_stop(3,6)
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (90,'(a,i5,a,f14.7)') " step ",it_corrente," * aggiornamento velocity tot ",cpu_dopo - cpu_prima
!
!=== FINE AGGIORNAMENTO VEL ############################
!
!...................................... 2011 mar 08
...
!
!.. smoothing of the velocity field in order to reduce oscillations
!.. depending on the smoothing coefficient of velocity ThetaV
!
      call start_and_stop(2,7)
!!!tempi    call cpu_time(cpu_prima)
!
...
!
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (91,'(a,i5,a,f14.7)') " step ",it_corrente," * calcolo smoothing velocity tot ",cpu_dopo - cpu_prima
!
!.. smoothing of the velocity
!
!!!tempi    call cpu_time(cpu_prima)
...
!
        if (pg(npi)%kodvel == 0) then                                            
!
! aggiorno tetav adeguato al passo temporale
!          TetaV1 = Domain%TetaV * Med(pg(npi)%imed)%Celerita * dt / Domain%h
!.. upgrade the velocity component array with the average velocity values previously evaluated
          pg(npi)%var(:) = pg(npi)%vel(:) + TetaV1 * pg(npi)%var(:)      
          pg(npi)%vel(:) = pg(npi)%var(:)                                        
...
!
      call start_and_stop(3,7)
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (92,'(a,i5,a,f14.7)') " step ",it_corrente," * aggiornamento smoothing velocity tot ",cpu_dopo - cpu_prima
!
!=== FINE CORREZIONE VELOCITA' ################
!
!
...
!.. if the diffusion model is set, the diffusion coefficient is upgraded
!
!.. computation of the diffusion coefficient (diffused interface model)
      if (diffusione) then
        call start_and_stop(2,15)
!
!$omp parallel do default(none) private(npi,ii,appo1,appo2,appo3) shared(nag,pg,Med,indarrayFlu,Array_Flu)
...
!
!.. loop to compute the trajectories
      do npi = 1,nag
!
        if (pg(npi)%cella == 0) cycle
!
...
!
        if (pg(npi)%vel_type /= "std") then
!.. the type of the velocity condition is "std", the assigned velocity condition is used
          pg(npi)%coord(:) = pg(npi)%coord(:) + dt * pg(npi)%vstart(:)
!
        else
!.. otherwise, the smoothed value V^ already calculated are used to define the particle path
...
!
      call start_and_stop(3,8)
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (93,'(a,i5,a,f14.7)') " step ",it_corrente," * Agg. Posizione tot ",cpu_dopo - cpu_prima
!
!
!.. checks for the particles go out of the domain throughout the opened sides
!
      call start_and_stop(2,9)
!
...
!
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (101,'(a,i5,a,f14.7)') " step ",it_corrente," * OrdGrid1 tot ",cpu_dopo - cpu_prima
!
!
!..??? CALCOLO QUANTITA' PER PARTICELLE FISSE - 20050516 ELIMINATO!
!
...
!.. evaluates the distances between the one particle and all the nearest ones, storing them 
!.. in the general storage array
!.. computation and storage of the neighbouring indeces, distances and kernel gradients
!
      call start_and_stop(2,10)
!!!tempi    call cpu_time(cpu_prima)
      call CalcVarLength
...
!
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (94,'(a,i5,a,f14.7)') " step ",it_corrente," * CalcVarLength tot ",cpu_dopo - cpu_prima
!
!.. evaluates the close boundaries and the integrals
!.. for the current particle in every loop, storing them in the general storage array
!.. computation and storage of the boundary integrals
...
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (95,'(a,i5,a,f14.7)') " step ",it_corrente," * ComputeBoundaryDataTab tot ",cpu_dopo - cpu_prima
!
!AA401
!.. staggered Euler option for time integration (velocity+E), 
!.. velocity smoothing, trajectory equation, BC, neighboring parameters (end)
...
!=== EQUAZIONE DI CONTINUITA' ################
!
!.. erosion criterium + continuity equation right hand side + update of the deformation tensor norm (erosion criterium)
    call start_and_stop(2,12)
!
    Ncbf_Max = 0
!
!!!tempi    call cpu_time(cpu_prima)
!
!!!tempi    call cpu_time(cpu_loop1a)
!!!tempi    write (96,'(a,f12.5,a,f14.7)') " * loop1 ",cpu_loop1a-cpu_prima
!
    if (erosione .and. .not. esplosione) then  ! erosione
!
      if (Domain%time_split == 1) then 
...
!
!!!tempi    call cpu_time(cpu_loop6a)
!!!tempi    write (96,'(a,f12.5,a,f14.7)') " * loop6 ",cpu_loop6a-cpu_loop5a
!
!$$$$$
!
!
...
!.. Contributo contorno alla derivata della densita' e aggiornamento della densita'
    if (Domain%tipo == "semi") then
!$omp parallel do default(none) &
!$omp private(npi,ii,BCtorodivV,Ncbf) &
...
!$$$$$
!
!.. Ricerca le facce di contorno vicine alla particella "npi"
      BCtorodivV = zero
      Ncbf = BoundaryDataPointer(1,npi)
!.. Seleziona le facce che danno effettivo contributo
...
!
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (96,'(a,f12.5,a,f14.7)') " * loop7 ",cpu_dopo-cpu_loop6a
!!!tempi    write (96,'(a,i5,a,f14.7)') " step ",it_corrente," * Eq. Continuita tot ",cpu_dopo - cpu_prima
!!!tempi    write (96,'(a)') " -----"
!
    if ( Ncbf_Max > Domain%MAXCLOSEBOUNDFACES ) then
      write (nout,"(a,i5,a,i5)") "Increase parameter MAXCLOSEBOUNDFACES from ",Domain%MAXCLOSEBOUNDFACES," to ",Ncbf_Max
...
!
!AA406 sub
!$omp parallel do default(none) private(npi,ii) shared(nag,pg,dt,indarrayFlu,Array_Flu,Domain,med)
!
      do ii = 1,indarrayFlu
        npi = Array_Flu(ii)
!$$$$$
...
!
      call start_and_stop(3,12)
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (97,'(a,i5,a,f14.7)') " step ",it_corrente," * Aggiornamento densita' ",cpu_dopo - cpu_prima
!
!=== FINE EQUAZIONE DI CONTINUITA' ################
!
!.. modello bifluido
...
!!!tempi    call cpu_time(cpu_prima)
      ...
      call start_and_stop(3,13)
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (98,'(a,i5,a,f14.7)') " step ",it_corrente," * CalcPre tot ",cpu_dopo - cpu_prima
!
...
!AA401 staggered Euler option for time integration of the continuity equation + state equation (end)
    end if   !if time_split
!
!=== FINE EQUAZIONE DI STATO ####################################
!
...
!
!!!tempi        call cpu_time(cpu_dopo)
!!!tempi        write (99,'(a,i5,a,f14.7)') " step ",it_corrente," * Smoothing Pressure tot ",cpu_dopo - cpu_prima
!!!!tempi        write (99,'(a,f12.5,a,f14.7)') " * loop1 ",cpu_loop1a-cpu_prima," * loop2 ",cpu_dopo-cpu_loop2a
!
      else if (Domain%Psurf == 'a') then
!
!.. set the pressure smoothed with the weight of atmospheric pressure and set the density
...
!
!!!tempi        call cpu_time(cpu_dopo)
!!!tempi        write (99,'(a,i5,a,f14.7)') " step ",it_corrente," * PressureSmoothing tot ",cpu_dopo - cpu_prima
!!!!tempi        write (99,'(a,f12.5,a,f14.7)') " * loop1 ",cpu_loop1b-cpu_loop1a," * loop2 ",cpu_loop2b-cpu_loop2a
!
      end if
!
      call start_and_stop(3,14)
...
!
!=== FINE CORREZIONE PRESSIONE PARTICELLE ################
!
...
!§
!
!=== FINE AGGIORNAMENTO DENSITA PARTICELLE ################
...
!.. apparent viscosity (erosion model)
      ...
      call start_and_stop(3,15)
!!!tempi    call cpu_time(cpu_dopo)
!!!tempi    write (100,'(a,i5,a,f14.7)') " step ",it_corrente," * Viscapp tot ",cpu_dopo - cpu_prima
    end if
!
!AA406
    endif
...
!AA403 start
!=== Boundary Conditions (start) ################
! BC (checks for the particles gone out the domain throughout the opened sides)
    if ((Domain%time_split == 0) .and. (Domain%time_stage == Domain%RKscheme)) then
      call start_and_stop(2,9)
      if (NumOpenFaces > 0) call CancelOutgoneParticles_3D
!.. adds the new particles from the source conditions
...
!AA504 start 
!Calling Update_Zmax_at_grid_vert_columns
     if (allocated(Z_fluid_max)) then
       if ((int(tempo/Domain%depth_dt_out)>Domain%depth_it_out_last).or.(it_corrente==1)) then
